...
private fun printHelp() {
  ...
  with(log) {
    ...
    info("  ports : Display a list of all available serial ports. Useful for setting the " +
      "${RobotControllerAdapter.serialPortEnvVar} and ${Button.serialPortEnvVar} environment variables.")
    info("")
    info("--------------------")
    info("")
    info("Usage examples:")
    ...
    info("    : Listen to hardware button clicks until the user presses Enter. If clicked, the button will run the robot demo, " +
      "but won't communicate with an Android device even if connected.")
    info("")
    info("  demo full")
